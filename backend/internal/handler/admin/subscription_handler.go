...
func (h *SubscriptionHandler) List(c *gin.Context) {
	...
	upstreamPlatform := c.Query("upstream_platform")

	// Parse sorting parameters
	sortBy := c.DefaultQuery("sort_by", "created_at")
	sortOrder := c.DefaultQuery("sort_order", "desc")

	subscriptions, pagination, err := h.subscriptionService.List(c.Request.Context(), page, pageSize, userID, groupID, status, upstreamPlatform, sortBy, sortOrder)
	if err != nil {
