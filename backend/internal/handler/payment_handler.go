...
func (h *PaymentHandler) GetPlans(c *gin.Context) {
	...
	for _, p := range plans {
		group := groupInfo[p.GroupID]
		result = append(result, planWithGroup{
			ID: int64(p.ID), GroupID: p.GroupID,
			GroupName: group.Name, GroupIcon: group.Icon, GroupColor: group.Color,
			UpstreamPlatforms: group.UpstreamPlatforms, UpstreamProtocols: group.UpstreamProtocols,
			AvailableIngressProtocols: group.AvailableIngressProtocols,
...
func (h *PaymentHandler) GetCheckoutInfo(c *gin.Context) {
	...
	for _, p := range plans {
		...
		planList = append(planList, checkoutPlan{
			ID: int64(p.ID), GroupID: p.GroupID,
			GroupName: gi.Name, GroupIcon: gi.Icon, GroupColor: gi.Color,
			UpstreamPlatforms: gi.UpstreamPlatforms, UpstreamProtocols: gi.UpstreamProtocols,
			AvailableIngressProtocols: gi.AvailableIngressProtocols,
