...
### 2.1 `openai` / `forward` / `http_openai` / `http_rpc`（OpenAI 兼容透传）
...
### 2.2 `openai_responses`（Responses 主协议）

- 适用：上游以 `/v1/responses` 为主
- 已实现端点：
  - `POST /v1/responses`
  - `POST /v1/chat/completions`（转换到 `/responses`）
  - `GET /v1/models`

### 2.3 `gemini`（OpenAI ↔ Gemini + Gemini Native）
...
### 2.4 `anthropic`（OpenAI ↔ Claude + 原生 messages）
...
### 2.5 `azure_openai`（Azure OpenAI 双栈）

- `v1` 栈：`/openai/v1/*`
- legacy 栈：`/openai/deployments/{deployment}/*?api-version=...`
- 对于 legacy 不支持的端点（例如 `/responses`）会返回 `not_supported`

### 2.6 `http_openai` / `http_rpc`（模块 HTTP 透传语义）
...
### 2.7 `codex`（OpenAI Chat/Responses ↔ Codex Responses 转换）
...
### 3.1 配置内置 Provider：`forward` / `anthropic`（或新增 openai/gemini/azure_openai）
...
- `openai` / `forward` / `http_openai` / `http_rpc` / `openai_responses` / `codex` / `azure_openai`
- `gemini`（走 `/v1beta/models`）
- `anthropic`（走 `/v1/models`）
