...
- `https://example.com/lightbridge/index.json`

要求：

- 可被 Core 直接通过 HTTP GET 拉取
- 内容为 ModuleIndex JSON（字段见参考文档）

### 1.3 GitHub 目录扫描（无需提前生成 index.json）

你可以把一个 GitHub 仓库里的某个目录当作“模块目录”，由 Core 通过 GitHub API 扫描其中的 `*.zip` 并即时生成 index：

写法 1（推荐）：

- `github:<owner>/<repo>/MODULES@main`
- `gh:<owner>/<repo>/MODULES@main`

写法 2（GitHub 仓库 tree URL）：

- `https://github.com/<owner>/<repo>/tree/<ref>/MODULES`
